...
class Engine:
    ...
    def compute_stock_path(self, volatilties,number_iterations):
        initial_stock = self.param.stock_initial
        rand = self.generate_random_by_step(number_iterations)
        stock_price = list()
        stock_price.append(initial_stock)
        ...
        for i in range(1, self.param.number_of_step + 1):
            ...
            current_price = previous_price* (1 + self.param.stock_return* self.param.dt + np.sqrt(volatilties[i - 1]) * ran[0]* math.sqrt(self.param.dt))
